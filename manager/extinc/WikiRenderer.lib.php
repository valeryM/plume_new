...
/**
 * Bibliotheque d'objets permettant de tranformer un texte, contenant des 
 ...
 * Thanks to all users who found bugs : 
 * Loic, Edouard Guerin, Sylvain, Ludovic L.
 *
...
class WikiInlineParser {
    ...
    function _parse($tag, &$posstart) {
        ...
        // on parcours la chaine,  morceau après morceau
...
class WikiRendererBloc {
    ...
    /**
     * @var boolean indique si le bloc doit être immediatement ferm� apr�s d�tection
     * @access protected
     */
    var $_closeNow=false;
...
class WikiRenderer {
    ...
    /**
     * instancie les différents objets pour le rendu des élements inline et bloc.
     */

    function WikiRenderer($config=null, $prefix='WRB_') {
        if(is_null($config))
