...
define('PX_RESOURCE_MANAGER_EVENTS', 'events');

/**
 * Data storage of a events.
 */
class Events extends Resource
{
    ...
    function load($id='')
    {
        ...
        if (PX_RESOURCE_MANAGER_EVENTS != $this->f('type_id'))
            return false;
    ...
    {
        $this->getConnection();
        $r = 'SELECT * FROM '.$this->con->pfx.'events
    ...
    /**
     * Return the content of the event as a string ready for indexation.
     *   
     * @param string Format of the string (html, wiki, text)
     * @return string The content of the event as a string
     */
    function getAsString($format='Html')
    {
    	...
    	$string .= ' '.$this->details->f('event_shortcontent');
        //$string .= ' '.$this->details->f('event_startdate');
        //$string .= ' '.$this->details->f('event_enddate');
        $string .= ' '.$this->details->f('event_content','Text');
        return $string;
    }

    /**
     * Set the default values for the event.
     *
    ...
    function setDefaults($user)
    {
        ...
        if ($user->getPref('events_status')) {
            $this->setField('status', $user->getPref('events_status'));
        } else {
            $this->setField('status', PX_RESOURCE_STATUS_INEDITION);
        }
        $this->setField('category_id', $user->getPref('events_category_id'));
        $this->setField('subtype_id', $user->getPref('events_subtype'));
     ...
     * @param int Subtype of the event
     */
...
/*
    ...
    /**
     * Set the details of a event.
     *
     * @param string Associated website title
     * @param string Associated website URL
     * @return bool Success
     */
    function setDetails($startdate, $enddate, $shortcontent, $content, $format, $useenddate)
    {
 		if (!empty($useenddate)) $enddate = date::EOT();
        if ($startdate == $enddate )  {
        	$enddate = date::EOT();
        } 		
        $this->details->setField('event_startdate', trim($startdate));
        $this->details->setField('event_enddate', trim($enddate));
        $this->details->setField('event_shortcontent', '='.$format."\n".trim($shortcontent));
        $this->details->setField('event_content', '='.$format."\n".trim($content));
    ...
    function check()
    {
        ...
        if (false === date::clean($this->f('enddate'))) {
            $this->setError($this->f('enddate').': '.__('The off-line date is wrong.'), 400);
        }
    ...
    {
        Hook::register('onInitTemplate', 'Events', 'hookOnInitTemplate');
        ...
        $GLOBALS['_PX_render']['events'] = '';
        $events =& $GLOBALS['_PX_render']['events']; 
        $GLOBALS['_PX_render']['ct'] = '';
        $ct =& $GLOBALS['_PX_render']['ct']; 

        // Parse query string to find the matching event
        list($events_id, $category_path) = Events::parseQueryString($query);
        // Load the matching event
        // If event does not exists, returns error code
        // Will be catched up by the 404 at the end
        
        $sql = SQL::getResourceInCat($events_id, $category_path, 
                                           config::f('website_id'));
       /*
        $sql = SQL::getResourcesInCat($events_id, $category_path,
        		config::f('website_id'));
        */
        $con =& pxDBConnect();
        if (($events = $con->select($sql, 'Events')) !== false) {
            if (!$events->isEmpty() && $events->f('type_id')=='events') {
                $events->load();
            } else {
        ...
        $cache = new Cache(urlencode('events%%'.$category_path.'%%'.$events_id));
        $cache->setCacheDirectory(config::getCacheDir());
        $cache->debug = config::f('debug');

        $GLOBALS['_PX_render']['ct'] = $events->comments;
        $GLOBALS['_PX_render']['res_id'] = $events_id;
        header(FrontEnd::getHeader($events->f('subtype_template')));
        // Load the template
		include config::f('manager_path').'/templates/'
            .config::f('theme_id').'/'.$events->f('subtype_template');
        return 200;
    }
    ...
    {
        if (config::f('action') == 'Events') {
            $GLOBALS['_PX_render']['cat'] = FrontEnd::getCategory($GLOBALS['_PX_render']['events']->f('category_id'));;
            ...
            $GLOBALS['_PX_render']['res'] = FrontEnd::getResources($GLOBALS['_PX_render']['events']->f('category_id'),
                                                                   '', '', 1, $order);
            $GLOBALS['_PX_render']['ct_enabled'] = false;
            if ((config::f('comment_support') == 1) 
                or (config::f('comment_support') == 2 && 
                    $GLOBALS['_PX_render']['events']->f('comment_support') == 1)) {
     ...
     * @return array (event id, category path)
     */
    ...
    function commit()
    {
        ...
        if (!$update) {
            //Insert, need to get the id of the event as the path and 
            ...
            $this->setField('identifier', 'events-'.$id);
    ...
    function commitDetails()
    {
        ...
        if ($update) {
            $req = 'UPDATE '.$this->con->pfx.'events SET
            	event_startdate = \''.$this->con->esc($this->details->f('event_startdate')).'\',
            	event_enddate = \''.$this->con->esc($this->details->f('event_enddate')).'\',
            	event_shortcontent = \''.$this->con->esc($this->details->f('event_shortcontent')).'\',
            	event_content = \''.$this->con->esc($this->details->f('event_content')).'\'
                WHERE resource_id = \''.$this->con->esc($this->f('resource_id')).'\'';
        } else {
            $req = 'INSERT INTO '.$this->con->pfx.'events SET
            	event_startdate = \''.$this->con->esc($this->details->f('event_startdate')).'\',
            	event_enddate = \''.$this->con->esc($this->details->f('event_enddate')).'\',
            	event_shortcontent = \''.$this->con->esc($this->details->f('event_shortcontent')).'\',
            	event_content = \''.$this->con->esc($this->details->f('event_content')).'\',
    ...
    function remove()
    {
        ...
        $delReq = 'DELETE FROM '.$this->con->pfx.'events WHERE
                   resource_id = \''.$this->con->esc($id).'\'';
        if (!$this->con->execute($delReq)) {
            $this->setError('MySQL: '.$this->con->error(), 500);
            return false;
    ...
    /**
     * Remove details.
     * Remove the detailed informations from the `events` table for 
     * the current event.
     *
     * @return bool Success
     */
    function removeDetails()
    {
        if (0 < (int) $this->f('resource_id')
            ...
            $this->getConnection();
            $req = 'DELETE FROM '.$this->con->pfx.'events WHERE 
               resource_id = \''.$this->con->esc($this->f('resource_id')).'\'';
            if (!$this->con->execute($req)) {
                $this->setError('MySQL: '.$this->con->error(), 500);
                return false;
            ...
            $this->setError(__('Error: No details to be removed for the current events.'), 400);
            return false;
        }
    }
}
