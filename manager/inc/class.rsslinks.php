...
define('PX_RESOURCE_MANAGER_RSSLINKS', 'rsslinks');

/**
 * Data storage of a rss links.
 */
class Rsslinks extends Resource
{
    ...
    function load($id='')
    {
        ...
        if (PX_RESOURCE_MANAGER_RSSLINKS != $this->f('type_id'))
            return false;
    ...
    /**
     * Get the details of a rss links.
    ...
    {
        $this->getConnection();
        $r = 'SELECT * FROM '.$this->con->pfx.'rsslinks 
    ...
    /**
     * Return the content of the rss links as a string ready for indexation.
     *   
    ...
    function getAsString($format='Html')
    {
        ...
        $string .= ' '.$this->details->f('rsslink_titlewebsite');
        //$string .= ' '.$this->details->f('rsslink_linkwebsite');
        return $string;
    }

    /**
     * Set the default values for the rss links.
     *
    ...
    function setDefaults($user)
    {
        ...
        if ($user->getPref('rsslinks_status')) {
            $this->setField('status', $user->getPref('rsslinks_status'));
        } else {
            $this->setField('status', PX_RESOURCE_STATUS_INEDITION);
        }
        $this->setField('category_id', $user->getPref('rsslinks_category_id'));
        $this->setField('subtype_id', $user->getPref('rsslinks_subtype'));
    ...
    /**
     * Set the basic data of a rss link.
    ...
    function setDetails($title, $link)
    {
        ...
        $this->details->setField('rsslink_titlewebsite', trim($title));
        $this->details->setField('rsslink_linkwebsite', trim($link));
        $this->isModified = true;
        if (false !== ($error = $this->error())) {
            return false;
        }
    ...
    /**
     * Check the integrity of the rss link.
     *
    ...
    function check()
    {
        ...
        if (strlen($this->f('rsslink_titlewebsite')) > 0) {
            if (strlen($this->f('news_linkwebsite'))
                && !preg_match('/^(http|ftp|news|https):\/\//', 
                               $this->f('rsslink_linkwebsite'))) {
                $this->setError(__('The associated link must start with http:// or a valid protocol.'), 400);
            }
        }      
    ...
    {
        Hook::register('onInitTemplate', 'Rsslinks', 'hookOnInitTemplate');
        ...
        $cache = new Cache(urlencode('rsslink%%'.$category_path.'%%'.$rss_id));
        $cache->setCacheDirectory(config::getCacheDir());
        $cache->debug = config::f('debug');

        $GLOBALS['_PX_render']['ct'] = $rsslinks->comments;
        $GLOBALS['_PX_render']['res_id'] = $rss_id;
        header(FrontEnd::getHeader($rsslinks->f('subtype_template')));
        // Load the template
		include config::f('manager_path').'/templates/'
            .config::f('theme_id').'/'.$rsslinks->f('subtype_template');
        return 200;
    }
    ...
    {
        if (config::f('action') == 'Rsslinks') {
            $GLOBALS['_PX_render']['cat'] = FrontEnd::getCategory($GLOBALS['_PX_render']['rsslinks']->f('category_id'));;
            ...
            $GLOBALS['_PX_render']['res'] = FrontEnd::getResources($GLOBALS['_PX_render']['rsslinks']->f('category_id'),
                                                                   '', '', 1, $order);
            $GLOBALS['_PX_render']['ct_enabled'] = false;
            if ((config::f('comment_support') == 1) 
                or (config::f('comment_support') == 2 && 
                    $GLOBALS['_PX_render']['rsslinks']->f('comment_support') == 1)) {
     ...
     * @return array (rsslink id, category path)
     */
    ...
    function commit()
    {
        ...
        if ($update) {
            ...
            $req = 'INSERT INTO '.$this->con->pfx.'resources SET
                ...
                type_id         = \''.PX_RESOURCE_MANAGER_RSSLINKS.'\',
                subtype_id      = \''.$this->con->esc($this->f('subtype_id')).'\',
                comment_support  = \''.$this->con->esc($this->f('comment_support')).'\',
        ...
        if (!$this->con->execute($req)) {
            $this->setError('MySQL: '.$this->con->error().' - '. $req, 500);
            return false;
        }
    ...
    function commitDetails()
    {
        ...
        if ($update) {
            $req = 'UPDATE '.$this->con->pfx.'rsslinks SET
                rsslink_serial = \''.$this->con->esc(md5($this->f('title'))).'\',
                rsslink_titlewebsite = \''.$this->con->esc($this->details->f('rsslink_titlewebsite')).'\',
                rsslink_linkwebsite = \''.$this->con->esc($this->details->f('rsslink_linkwebsite')).'\'
                WHERE resource_id = \''.$this->con->esc($this->f('resource_id')).'\'';
        } else {
            $req = 'INSERT INTO '.$this->con->pfx.'rsslinks SET
                rsslink_serial = \''.$this->con->esc(md5($this->f('title'))).'\',
                rsslink_titlewebsite = \''.$this->con->esc($this->details->f('rsslink_titlewebsite')).'\',
                rsslink_linkwebsite = \''.$this->con->esc($this->details->f('rsslink_linkwebsite')).'\',
    ...
    /**
     * Remove the rss links from the database.
     *
     * @return bool Success
     */
    function remove()
    {
        ...
        $delReq = 'DELETE FROM '.$this->con->pfx.'rsslinks WHERE
                   resource_id = \''.$this->con->esc($id).'\'';
        if (!$this->con->execute($delReq)) {
            $this->setError('MySQL: '.$this->con->error(), 500);
            return false;
    ...
    /**
     * Remove details.
     * Remove the detailed informations from the `rsslinks` table for 
     * the current news.
     *
     * @return bool Success
     */
    function removeDetails()
    {
        if (0 < (int) $this->f('resource_id')
            ...
            $this->getConnection();
            $req = 'DELETE FROM '.$this->con->pfx.'rsslinks WHERE 
               resource_id = \''.$this->con->esc($this->f('resource_id')).'\'';
            if (!$this->con->execute($req)) {
                $this->setError('MySQL: '.$this->con->error(), 500);
                return false;
            ...
            $this->setError(__('Error: No details to be removed for the current rss link.'), 400);
            return false;
        }
    }
}
