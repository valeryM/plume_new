<?php
	...
	*/
	...
	if (isset($_GET['heightMax'])) {
		$output_heightMax = $_GET['heightMax'];
		
		//	Have they given us a percentage?
		if (substr($output_heightMax, -1) == '%')
		{
			//	Yes, remove the % sign
			$output_heightMax = (int) substr($output_heightMax, 0, -1);
			$output_heightMax = $output_heightMax / 100;
		}
	}
	...
	*/
	...
	if ($output_heightMax>0 && $height>$output_heightMax) {
		if ($output_heightMax<1) {	// valeur en %
			$dst_width = $width * $output_heightMax;
			$dst_height = $height * $output_heightMax;
		} else {	// valeur num : on recalcul la taille en respectant le ratio
			if ($width > $height) {
				$dst_height = $output_heightMax*80/100;
			} else {
				$dst_height = $output_heightMax;
			}
			$dst_width = ($width / $height)*$dst_height;
		}
		$tmp_out = imagecreatetruecolor($dst_width, $dst_height);
		
		imagecopyresampled($tmp_out,$source,0,0,0,0,$dst_width,$dst_height,$width,$height);
		$source = $tmp_out;
		unset($tmp_out);
		$width = $dst_width;
		$height = $dst_height;
