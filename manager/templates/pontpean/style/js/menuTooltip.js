...
$(document).ready(function() {
	...
	$(function() {$(".menuToolTip").corner("5px");});
	
	var afficheMenu = false;
	var configLittleMenu = {
		over : littleMenuOver, // function = onMouseOver callback (REQUIRED)
		timeout : 500, // number = milliseconds delay before onMouseOut
		out : littleMenuOut // function = onMouseOut callback (REQUIRED)
	};

	$(".menuFlottant").hoverIntent(configLittleMenu);

	function littleMenuOver() {
		afficheMenu = true;
		$(".menuToolTip").fadeOut("fast");
		$(".menuSousCategorie").fadeOut("fast");
		var pos = $(this).findPos();
		var decY = -55;
		//console.log( navigator.userAgent);
		if ($.browser.msie && ($.browser.version == '6.0' || $.browser.version == '7.0' || $.browser.version == '8.0') ) {
			var largeur = $(this).width();
			$(this).children("table").css({marginLeft : "-" + largeur + "px"});
			$(this).children(".menuToolTip").css("top", pos.y + pos.height + decY +20);
			$(this).children(".menuToolTip").css("left", "25px");
		} else {
			$(this).children(".menuToolTip").css("top", pos.y + pos.height + (decY/2));
			$(this).children(".menuToolTip").css("left", "0px");
		}
		$(this).children(".menuToolTip").pause(50).slideDown("normal");
		
	}

	function littleMenuOut() {
		if (afficheMenu == false) {
			$(this).children(".menuToolTip").fadeOut("fast");
		}		
	}
	...
	$(".menuPrincipal").hoverIntent(configMenu);

	function menuOver() {
		afficheMenu = true;
		$(".menuToolTip").fadeOut("fast");
		var pos = $(this).children("a").findPos();
		//var pos = $(this).findPos();
		//var decY = 120;
		var decY = 30;
		//console.log( navigator.userAgent);
		if ($.browser.msie && ($.browser.version == '6.0' || $.browser.version == '7.0' || $.browser.version == '8.0') ) {
			var largeur = $(this).width();
			$(this).children(".menuToolTip").css({marginLeft : "-" + largeur + "px"});
			$(this).children(".menuToolTip").css("top", pos.y + pos.height + decY);
			//$(this).children(".menuToolTip").css("left", pos.x + pos.width);
			
		} else {
			$(this).children(".menuToolTip").css("top", pos.y + decY);
			//$(this).children(".menuToolTip").css("left", pos.x);
		}
		$(this).children(".menuToolTip").pause(50).slideDown("normal");
	}
	...
	$(".menuPrincipal table").mouseover(function() {
		afficheMenu = true;
	});
	
	$(".menuPrincipal table").mouseout(function() {
		afficheMenu = false;
	});
	/*
	...
	*/
	$(".menuFlottant table").mouseover(function() {
		afficheMenu = true;
	});
	$(".menuFlottant table").mouseout(function() {
		afficheMenu = false;
	});
	
	$("#menu_top.menu, div#banner2 .imgLeft").mouseover(function() {
		afficheMenu = false;
	});
	
	var configCat = {
		over : catOver, // function = onMouseOver callback (REQUIRED)
		timeout : 500, // number = milliseconds delay before onMouseOut
		out : catOut // function = onMouseOut callback (REQUIRED)
	};
	$("td.cadreMenu").hoverIntent(configCat);
	...
	function catOver() {
		...
		var pos = $(this).findPos();
		afficheMenu = true;
		var menu = $("#mSC_"+$(this).attr('id').substring(3));
		if ($.browser.msie) {
			var largeur = $(this).width();
			menu.css({marginLeft : "-" + largeur + "px"});
			menu.css({width:"200px"});
			menu.css("top", pos.y + pos.height);
			menu.css("left", pos.x + pos.width);
	...
	$(".menuSousCategorie").mouseover(function() {
		afficheMenu = true;
	});

	$(".menuSousCategorie").mouseout(function() {
		afficheMenu = false;
	});
	...
	$(".souscategorielink").mouseover(function() {
		classSousCategorie = $(this).children("a").attr("class");
		$(this).parent().attr("class","souscategorieSelected");
		$(this).children("a").attr("class","souscategorieSelected");
	});

	$(".souscategorielink").mouseout(function() {
		$(this).parent().attr("class", classSousCategorie);
		$(this).children("a").attr("class",classSousCategorie);
	});
	...
	instanceOne.init({
		...
		reflections : false,
		reflectionPNG : true,
		reflectionGET : '&height=15%&cache=' + true,
		reflectionP : 0.2,
		reflectPath : '/jquery/imageFlow/',
		circular : true,
		captions : false
	});
});
...
/**
 * Extension des fonctionnalités de JQuery Ajout de d'une fonction qui permet de
 * récupérer un objet DOM et de retourner un objet contenant les informations
...
jQuery.fn.extend({
	findPos : function() {
		...
		var curwidth = this.width() || 0;
		var curheight = this.height() || 0;
