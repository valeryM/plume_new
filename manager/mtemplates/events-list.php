...
if (basename($_SERVER['SCRIPT_NAME']) == 'events-list.php') exit;
...
if ($res->isEmpty()) {
    echo '<p class="noresource">'. __('No events.').'</p>'."\n\n";
} else {
    ...
    while (!$res->EOF()) {
    	...
    	if ($m->asRightToView($res))  {	
	        ...
	        */
	        echo '<p class="resource_title"><span class="events_style">'.$res->f('title').'</span> - '.__('by');
	        ...
	        echo '<span class="date-time">'.date( __('Y/m/d at H:i:s'),date::unix($res->getField('event_startdate')));
 ...
 * ================================================= */
echo '<form action="events.php" method="get"><p id="search">';
echo '<label for="q" style="display:inline;"><strong>'.
__('Search for a event item:').' </strong></label>';
