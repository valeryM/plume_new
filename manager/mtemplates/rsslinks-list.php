...
if (basename($_SERVER['SCRIPT_NAME']) == 'rsslinks-list.php') exit;


echo '<form action="rsslinks.php" method="get">';
...
echo '<input type="hidden" name="p" value="rsslinks">';
...
if ($res->isEmpty()) {
    echo '<p class="noresource">'. __('No rss links.').'</p>'."\n\n";
} else {
    ...
    while (!$res->EOF()) {
    	if ($m->asRightToView($res))  {	//$m->asRightToView($res)
	        ...
	        echo '<div class="resourcebox '.$res_class.'" id="p'.$res->f('resource_id').'">'.
	            ...
	            'alt="'. __('show/hide').'" /></a> ';
	        echo $res_img;
	        echo '<p class="resource_title"><span class="rsslinks_style">'.$res->f('title').'</span> - '.__('by');
...
/* ================================================= *
 *            Form to search in the rss link         *
 * ================================================= */
echo '<form action="rsslinks.php" method="get"><p id="search">';
echo '<label for="q" style="display:inline;"><strong>'.
__('Search for a rss link item:').' </strong></label>';
