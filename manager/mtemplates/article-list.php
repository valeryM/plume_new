...
if (basename($_SERVER['SCRIPT_NAME']) == 'article-list.php') exit;
...
//$valueLocation = '';
//if (!empty($_GET['location'])) $valueLocation = $_GET['location'];
...
if ($res->isEmpty()) {
	...
	while (!$res->EOF()) {
		if ($m->asRightToView($res))  {	//$m->asRightToView($res)
			...
			*/
			echo '<p class="resource_title"><span class="art_style">'.$res->f('title').'</span> - '.__('by');
...
=================================================*/
echo '<form action="articles.php" method="get"><p id="search">';
