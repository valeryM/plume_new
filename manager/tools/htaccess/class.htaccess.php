 ...
 *
 * @contributor loic d'Anterroches
 */
...
class htaccess
{
    ...
    var $defaultName='No name'; // Default name of the user
    var $defaultEmail='No email'; // Default email
     ...
     * @param string $filename    the name of htpasswd file
     */
    ...
    /**
     * Return true if both a .htaccess and a .htpasswd are in the folder
     * @param string folder
     */
    function isSecured($folder) 
    {
        $folder = preg_replace('#(/)*$#', '', $folder); 
        if (file_exists($folder.'/.htaccess')) {
            $this->setFHtaccess($folder.'/.htaccess');
            $this->setFPasswd($folder.'/.htpasswd');                        
            return true;
        }           
        return false;           
    }   
     ...
     * @param string $name   Real name of the user (optional)
     ...
     */
    function addUser($username,$password,$name='',$email='',$group='')
    {
        ...
        if($isAlready==false){
            ...
            if (strlen($name)) $newLine .= ':'.str_replace(':',' ',$name);
            if (strlen($email)) $newLine .= ':'.str_replace(':',' ',$email);
            $newLine .= "\n";
            fputs($file,$newLine);
            fclose($file);
            return true;
    ...
    {
        if (file_exists($this->fHtaccess)) {
            $f = fopen($this->fHtaccess, 'r');
            $file = fread($f, filesize($this->fHtaccess));
            fclose($f); 
            $match = '/AuthName\s+\"([^"]*)\"/mU';  
            if (preg_match($match,$file,$res)) {
                return $res[1];
            }
            return '';
        }
